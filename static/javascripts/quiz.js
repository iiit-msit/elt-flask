...
var questionView = {
		...
		displayOptions : function() {
			var q = quizModel.question;
			var optionsHTML = '';
			console.log(['questionType', q.questionType, ["", "radio"].indexOf(q.questionType)]);
			if([undefined, "", "radio"].indexOf(q.questionType)>=0){
			...
			}else if(["checkbox"].indexOf(q.questionType)>=0){
				console.log("found checkbox");
				for (var i = 0; i < q.options.length; i++) {
					var optionText = q.options[i];
					optionsHTML += '<div class="checkbox">';
					optionsHTML += '<label><input type="checkbox" name="optionsRadios" id="optionsRadios1" value="' + optionText + '"';
				...
				alert("unknown question type. Please contact admin.")
			}
		},
